...
redshift = st.sidebar.slider("Redshift", 0.5, 3.0, 1.0)
met = st.sidebar.slider("Metallicity", -1.5, 0.5, 0.0)
dust = st.sidebar.slider("Dust Attenuation", 0.0, 1.0, 0.1)

image = st.empty()

rand_spec, rand_lam = db.make_spec(rand_sfh_tuple, met, dust, redshift, return_lam = True)
fig = plt.plot(rand_lam*(1+redshift), rand_spec)
plt.xscale('log');plt.xlim(1e3,1e5)
plt.yscale('log');plt.ylim(1e-3,1e7)
